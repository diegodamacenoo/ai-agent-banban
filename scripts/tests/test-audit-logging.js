...
async function testAuditLogging() {
  ...
  try {
    // 1. Verificar conexao
    console.log('1. Verificando conexao com Supabase...');
    ...
    if (usersError) {
      console.error('❌ Erro na conexao:', usersError.message);
      return;
    }
    
    if (!users || users.length === 0) {
      console.log('⚠️ Nenhum usuario encontrado na tabela profiles');
      return;
    }
    ...
    console.log('✅ Tabela audit_logs acessivel');
    
    // 3. Verificar politicas RLS (tentativa de insercao simples)
    console.log('\n3. Testando insercao de log de auditoria...');
    ...
    if (insertError) {
      ...
      if (insertError.code === '42501') {
        console.log('\n💡 Dica: Erro RLS detectado. Execute as migracoes:');
        console.log(`
-- Criar politicas RLS necessarias
...
WITH CHECK (true);
    ...
    console.log('\n🎉 Sistema de audit logging esta funcionando corretamente!');
...
async function testAuditLog() {
  console.log('Testando funcao createAuditLog...');
