...
Write-Host "=== TESTE DE VALIDACAO - SALES FLOW ===" -ForegroundColor Cyan
...
function Test-ValidationCase {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Uri $SALES_FLOW_URL -Method POST -Headers $Headers -Body $body -ContentType "application/json" -ErrorAction Stop
        $actualStatus = 200
