...
exports.BanBanTransferFlowService = void 0;
const supabase_js_1 = require("@supabase/supabase-js");
const index_1 = require("../../../../../shared/webhook-base/index");
const eca_actions_1 = require("../../../../../shared/enums/eca-actions");
const eca_types_1 = require("../../../../../shared/enums/eca-types");
class BanBanTransferFlowService {
    constructor() {
        const supabaseUrl = process.env.SUPABASE_URL;
        const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
        this.supabase = (0, supabase_js_1.createClient)(supabaseUrl, supabaseKey, { auth: { autoRefreshToken: false, persistSession: false } });
    }
    async processAction(action, transactionData, metadata) {
        await (0, index_1.validateAndGetTenant)(eca_types_1.BANBAN_ORG_ID);
        switch (action) {
            case eca_actions_1.TRANSFER_ACTIONS.CREATE_TRANSFER_REQUEST:
                return this._createTransferRequest(transactionData, metadata);
            case eca_actions_1.TRANSFER_ACTIONS.REGISTER_REQUEST:
                return this._registerRequest(transactionData, metadata);
            case eca_actions_1.TRANSFER_ACTIONS.CREATE_SEPARATION_MAP:
                return this._createSeparationMap(transactionData, metadata);
            case eca_actions_1.TRANSFER_ACTIONS.START_SEPARATION:
                return this._startSeparation(transactionData, metadata);
            case eca_actions_1.TRANSFER_ACTIONS.COMPLETE_SEPARATION:
                return this._completeSeparation(transactionData, metadata);
            case eca_actions_1.TRANSFER_ACTIONS.SHIP_TRANSFER:
                return this._registerShipment(transactionData, metadata);
            case eca_actions_1.TRANSFER_ACTIONS.INVOICE_TRANSFER:
                return this._invoiceTransfer(transactionData, metadata);
            case eca_actions_1.TRANSFER_ACTIONS.START_STORE_CONFERENCE:
                return this._startStoreConference(transactionData, metadata);
            case eca_actions_1.TRANSFER_ACTIONS.SCAN_STORE_ITEMS:
                return this._scanStoreItems(transactionData, metadata);
            case eca_actions_1.TRANSFER_ACTIONS.COMPLETE_STORE_CONFERENCE:
                return this._completeStoreConference(transactionData, metadata);
            case eca_actions_1.TRANSFER_ACTIONS.EFFECTUATE_STORE:
                return this._effectuateStore(transactionData, metadata);
            case eca_actions_1.TRANSFER_ACTIONS.REGISTER_RECEIPT:
                return this._registerReceipt(transactionData, metadata);
            case eca_actions_1.TRANSFER_ACTIONS.REGISTER_COMPLETION:
                return this._registerCompletion(transactionData, metadata);
            default:
                throw new Error(`Ação de transferência desconhecida: ${String(action)}`);
        }
    }
    async getTransferData(queryParams) {
        ...
        let query = this.supabase
            ...
            .select('*')
            .eq('organization_id', eca_types_1.BANBAN_ORG_ID)
        ...
        if (origin_location_external_id)
            query = query.contains('attributes', { origin_location_external_id: origin_location_external_id });
        if (destination_location_external_id)
            query = query.contains('attributes', { destination_location_external_id: destination_location_external_id });
        if (date_from)
        ...
        return {
            ...
            metadata: {
                ...
                organization_id: eca_types_1.BANBAN_ORG_ID,
                filters_applied: {
    ...
    async getTransferAnalytics(queryParams) {
        ...
        const transferOutCount = data.filter((item) => item.transaction_type === 'TRANSFER_OUT').length;
        const transferInCount = data.filter((item) => item.transaction_type === 'TRANSFER_IN').length;
    ...
    _calculateAvgTransferTime(data) {
        if (!data.length)
            return 0;
        const withStateHistory = data.filter(item => item.attributes?.state_history && item.attributes.state_history.length > 0);
        if (!withStateHistory.length)
            return 0;
    ...
    _calculateTopRoutes(data) {
        const routeCount = data.reduce((acc, item) => {
    ...
    _calculateDailyTransferVolume(data) {
    ...
    async _calculateDemandAnalysis(params) {
        return {
    ...
    async _createTransferRequest(data, metadata) {
        ...
        const originLocationEntity = await this._getOrCreateBusinessEntity(eca_types_1.BANBAN_ORG_ID, 'LOCATION', data.origin_location_external_id, { name: data.origin_location_name || `Local ${data.origin_location_external_id}` });
        const destinationLocationEntity = await this._getOrCreateBusinessEntity(eca_types_1.BANBAN_ORG_ID, 'LOCATION', data.destination_location_external_id, { name: data.destination_location_name || `Local ${data.destination_location_external_id}` });
        const transaction = await (0, index_1.createBusinessTransaction)(eca_types_1.BANBAN_ORG_ID, 'TRANSFER_OUT', data.external_id, { ...(data || {}), status: 'PEDIDO_TRANSFERENCIA_CRIADO' });
        await (0, index_1.createBusinessRelationship)(eca_types_1.BANBAN_ORG_ID, 'ORIGINATES_FROM', transaction.id, originLocationEntity.id, {});
        await (0, index_1.createBusinessRelationship)(eca_types_1.BANBAN_ORG_ID, 'DESTINED_TO', transaction.id, destinationLocationEntity.id, {});
        for (const item of data.items) {
            const productEntity = await this._getOrCreateBusinessEntity(eca_types_1.BANBAN_ORG_ID, 'PRODUCT', item.product_external_id || item.product_id, { name: item.product_name || `Produto ${item.product_external_id || item.product_id}` });
            await (0, index_1.createBusinessRelationship)(eca_types_1.BANBAN_ORG_ID, 'CONTAINS_ITEM', transaction.id, productEntity.id, { quantity: item.quantity, unit_cost: item.unit_cost });
        }
    ...
    async _registerRequest(data, metadata) {
    ...
    async _createSeparationMap(data, metadata) {
    ...
    async _startSeparation(data, metadata) {
        const transferExternalId = data.transfer_external_id || data.external_id;
        const transaction = await this._getTransactionByExternalId(transferExternalId, 'TRANSFER_OUT');
    ...
    async _completeSeparation(data, metadata) {
        const transferExternalId = data.transfer_external_id || data.external_id;
        if (!transferExternalId || !data.items)
            throw new Error('transfer_external_id e items são obrigatórios.');
        const transaction = await this._getTransactionByExternalId(transferExternalId, 'TRANSFER_OUT');
    ...
    async _registerShipment(data, metadata) {
        const transferExternalId = data.transfer_external_id || data.external_id;
        if (!transferExternalId || !data.items_shipped)
            throw new Error('transfer_external_id e items_shipped são obrigatórios.');
        const transaction = await this._getTransactionByExternalId(transferExternalId, 'TRANSFER_OUT');
        ...
        const originLocationEntity = await this._getOrCreateBusinessEntity(eca_types_1.BANBAN_ORG_ID, 'LOCATION', transaction.attributes.origin_location_external_id, { name: transaction.attributes.origin_location_name });
        for (const item of data.items_shipped) {
            const productEntity = await this._getOrCreateBusinessEntity(eca_types_1.BANBAN_ORG_ID, 'PRODUCT', item.variant_external_id, { name: item.product_name || `Produto ${item.variant_external_id}` });
            const inventoryMovement = await (0, index_1.createBusinessTransaction)(eca_types_1.BANBAN_ORG_ID, 'INVENTORY_MOVEMENT', null, {
            ...
            await (0, index_1.createBusinessRelationship)(eca_types_1.BANBAN_ORG_ID, 'AFFECTS_PRODUCT', inventoryMovement.id, productEntity.id, {});
            await (0, index_1.createBusinessRelationship)(eca_types_1.BANBAN_ORG_ID, 'AT_LOCATION', inventoryMovement.id, originLocationEntity.id, {});
            await (0, index_1.createBusinessRelationship)(eca_types_1.BANBAN_ORG_ID, 'CAUSED_BY_TRANSFER', inventoryMovement.id, transaction.id, {});
            await this._updateInventorySnapshot(item.variant_external_id, transaction.attributes.origin_location_external_id, -item.qty_shipped, 'TRANSFER_OUT', inventoryMovement.id);
        }
        return {
            transaction_id: transaction.id,
            external_id: transaction.external_id,
    ...
    async _invoiceTransfer(data, metadata) {
        const transferExternalId = data.transfer_external_id || data.external_id;
        if (!transferExternalId || !data.invoice_external_id)
            throw new Error('transfer_external_id e invoice_external_id são obrigatórios.');
        const transaction = await this._getTransactionByExternalId(transferExternalId, 'TRANSFER_OUT');
        ...
        if (error)
            throw error;
        const transferInDocument = await (0, index_1.createBusinessTransaction)(eca_types_1.BANBAN_ORG_ID, 'TRANSFER_IN', data.invoice_external_id, { ...data, status: 'AGUARDANDO_CONFERENCIA_LOJA', reference_transfer_id: transaction.id });
        await (0, index_1.createBusinessRelationship)(eca_types_1.BANBAN_ORG_ID, 'BASED_ON_ORDER', transferInDocument.id, transaction.id, {});
    ...
    async _startStoreConference(data, metadata) {
    ...
    async _scanStoreItems(data, metadata) {
    ...
    async _completeStoreConference(data, metadata) {
        if (!data.invoice_external_id || !data.items_received)
            throw new Error('invoice_external_id e items_received são obrigatórios.');
        const transaction = await this._getTransactionByExternalId(data.invoice_external_id, 'TRANSFER_IN');
        if (transaction.status !== 'EM_CONFERENCIA_LOJA') {
    ...
    async _effectuateStore(data, metadata) {
        if (!data.invoice_external_id)
            throw new Error('invoice_external_id é obrigatório.');
        const transaction = await this._getTransactionByExternalId(data.invoice_external_id, 'TRANSFER_IN');
    ...
    async _registerReceipt(data, metadata) {
        const transferExternalId = data.transfer_external_id || data.external_id;
        const transaction = await this._getTransactionByExternalId(transferExternalId, 'TRANSFER_OUT');
    ...
    async _registerCompletion(data, metadata) {
        const transferExternalId = data.transfer_external_id || data.external_id;
        if (!transferExternalId || !data.items_received)
            throw new Error('transfer_external_id e items_received são obrigatórios.');
        const transaction = await this._getTransactionByExternalId(transferExternalId, 'TRANSFER_OUT');
        ...
        const destinationLocationEntity = await this._getOrCreateBusinessEntity(eca_types_1.BANBAN_ORG_ID, 'LOCATION', transaction.attributes.destination_location_external_id, { name: transaction.attributes.destination_location_name });
        for (const item of data.items_received) {
            const productEntity = await this._getOrCreateBusinessEntity(eca_types_1.BANBAN_ORG_ID, 'PRODUCT', item.product_id, { name: item.product_name || `Produto ${item.product_id}` });
            const inventoryMovement = await (0, index_1.createBusinessTransaction)(eca_types_1.BANBAN_ORG_ID, 'INVENTORY_MOVEMENT', null, {
            ...
            await (0, index_1.createBusinessRelationship)(eca_types_1.BANBAN_ORG_ID, 'AFFECTS_PRODUCT', inventoryMovement.id, productEntity.id, {});
            await (0, index_1.createBusinessRelationship)(eca_types_1.BANBAN_ORG_ID, 'AT_LOCATION', inventoryMovement.id, destinationLocationEntity.id, {});
            await (0, index_1.createBusinessRelationship)(eca_types_1.BANBAN_ORG_ID, 'CAUSED_BY_TRANSFER', inventoryMovement.id, transaction.id, {});
            await this._updateInventorySnapshot(item.product_id, transaction.attributes.destination_location_external_id, item.qty_received, 'TRANSFER_IN', inventoryMovement.id);
        }
    ...
    async _updateAnalyticsSnapshots(transferData) {
    ...
    async _calculateRoutePerformance(originId, destId) {
        try {
        ...
        `)
            ...
            const routeTransfers = transferData.filter((transfer) => {
                ...
                const locations = relationships
                    .filter((rel) => rel.tenant_business_entities?.entity_type === 'LOCATION')
                    .map((rel) => rel.tenant_business_entities.external_id);
            ...
            const leadTimes = routeTransfers
                .filter((transfer) => transfer.updated_at && transfer.created_at)
                .map((transfer) => {
            ...
            const avgLeadTime = leadTimes.length > 0
                ...
                : 0;
            const onTimeTransfers = leadTimes.filter((time) => time <= 48).length;
            const onTimeRate = leadTimes.length > 0
                ? (onTimeTransfers / leadTimes.length) * 100
                : 0;
            const accurateTransfers = routeTransfers.filter((transfer) => {
    ...
    async _analyzeDemandPatterns(destId) {
        try {
            const { data: transfers, error } = await this.supabase
                .from('tenant_business_transactions')
                .select(`
        ...
        `)
            ...
            const transferData = transfers || [];
            const locationTransfers = transferData.filter((transfer) => {
                const relationships = Array.isArray(transfer.tenant_business_relationships)
                    ? transfer.tenant_business_relationships
                    : transfer.tenant_business_relationships ? [transfer.tenant_business_relationships] : [];
                return relationships.some((rel) => rel.tenant_business_entities?.entity_type === 'LOCATION' &&
            ...
            for (const transfer of locationTransfers) {
                ...
                for (const rel of relationships) {
                    if (rel?.tenant_business_entities && !Array.isArray(rel.tenant_business_entities) && rel.tenant_business_entities.entity_type === 'PRODUCT') {
    ...
    async _getTransactionByExternalId(externalId, transactionType = 'TRANSFER_OUT') {
        const { data, error } = await this.supabase.from('tenant_business_transactions').select('*').eq('organization_id', eca_types_1.BANBAN_ORG_ID).eq('external_id', externalId).eq('transaction_type', transactionType).single();
        if (error || !data)
    ...
    async _updateInventorySnapshot(variantExternalId, locationExternalId, qtyChange, movementType, referenceId) {
        ...
        const { data: existing } = await this.supabase.from('tenant_snapshots').select('*').eq('organization_id', eca_types_1.BANBAN_ORG_ID).eq('snapshot_key', snapshotKey).single();
...
exports.BanBanTransferFlowService = BanBanTransferFlowService;
//# sourceMappingURL=banban-transfer-flow-service.js.map