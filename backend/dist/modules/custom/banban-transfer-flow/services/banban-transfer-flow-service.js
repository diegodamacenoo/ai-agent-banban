...
class BanBanTransferFlowService {
    ...
    async processAction(action, transactionData, metadata) {
        await (0, index_1.validateAndGetTenant)(enums_1.BANBAN_ORG_ID);
        switch (action) {
            case enums_1.TRANSFER_ACTIONS.CREATE_TRANSFER_REQUEST:
                return this._createTransferRequest(transactionData, metadata);
            case enums_1.TRANSFER_ACTIONS.REGISTER_REQUEST:
                return this._registerRequest(transactionData, metadata);
            case enums_1.TRANSFER_ACTIONS.CREATE_SEPARATION_MAP:
                return this._createSeparationMap(transactionData, metadata);
            case enums_1.TRANSFER_ACTIONS.START_SEPARATION:
                return this._startSeparation(transactionData, metadata);
            case enums_1.TRANSFER_ACTIONS.COMPLETE_SEPARATION:
                return this._completeSeparation(transactionData, metadata);
            case enums_1.TRANSFER_ACTIONS.SHIP_TRANSFER:
                return this._registerShipment(transactionData, metadata);
            case enums_1.TRANSFER_ACTIONS.INVOICE_TRANSFER:
                return this._invoiceTransfer(transactionData, metadata);
            case enums_1.TRANSFER_ACTIONS.START_STORE_CONFERENCE:
                return this._startStoreConference(transactionData, metadata);
            case enums_1.TRANSFER_ACTIONS.SCAN_STORE_ITEMS:
                return this._scanStoreItems(transactionData, metadata);
            case enums_1.TRANSFER_ACTIONS.COMPLETE_STORE_CONFERENCE:
                return this._completeStoreConference(transactionData, metadata);
            case enums_1.TRANSFER_ACTIONS.EFFECTUATE_STORE:
                return this._effectuateStore(transactionData, metadata);
            case enums_1.TRANSFER_ACTIONS.REGISTER_RECEIPT:
                return this._registerReceipt(transactionData, metadata);
            case enums_1.TRANSFER_ACTIONS.REGISTER_COMPLETION:
    ...
    async getTransferData(queryParams) {
        ...
        let query = this.supabase
            ...
            .select('*')
            .eq('organization_id', enums_1.BANBAN_ORG_ID)
        ...
        return {
            ...
            metadata: {
                ...
                organization_id: enums_1.BANBAN_ORG_ID,
                filters_applied: {
    ...
    async _createTransferRequest(data, metadata) {
        ...
        const originLocationEntity = await this._getOrCreateBusinessEntity(enums_1.BANBAN_ORG_ID, 'LOCATION', data.origin_location_external_id, { name: data.origin_location_name || `Local ${data.origin_location_external_id}` });
        const destinationLocationEntity = await this._getOrCreateBusinessEntity(enums_1.BANBAN_ORG_ID, 'LOCATION', data.destination_location_external_id, { name: data.destination_location_name || `Local ${data.destination_location_external_id}` });
        const transaction = await (0, index_1.createBusinessTransaction)(enums_1.BANBAN_ORG_ID, 'TRANSFER_OUT', data.external_id, { ...(data || {}), status: 'PEDIDO_TRANSFERENCIA_CRIADO' });
    ...
    async _registerShipment(data, metadata) {
        ...
        const originLocationEntity = await this._getOrCreateBusinessEntity(enums_1.BANBAN_ORG_ID, 'LOCATION', transaction.attributes.origin_location_external_id, { name: transaction.attributes.origin_location_name });
        for (const item of data.items_shipped) {
            const productEntity = await this._getOrCreateBusinessEntity(enums_1.BANBAN_ORG_ID, 'PRODUCT', item.variant_external_id, { name: item.product_name || `Produto ${item.variant_external_id}` });
            const inventoryMovement = await (0, index_1.createBusinessTransaction)(enums_1.BANBAN_ORG_ID, 'INVENTORY_MOVEMENT', null, {
            ...
            await (0, index_1.createBusinessRelationship)(enums_1.BANBAN_ORG_ID, 'AFFECTS_PRODUCT', inventoryMovement.id, productEntity.id, {});
            await (0, index_1.createBusinessRelationship)(enums_1.BANBAN_ORG_ID, 'AT_LOCATION', inventoryMovement.id, originLocationEntity.id, {});
            await (0, index_1.createBusinessRelationship)(enums_1.BANBAN_ORG_ID, 'CAUSED_BY_TRANSFER', inventoryMovement.id, transaction.id, {});
    ...
    async _invoiceTransfer(data, metadata) {
        ...
        if (error)
            throw error;
        const transferInDocument = await (0, index_1.createBusinessTransaction)(enums_1.BANBAN_ORG_ID, 'TRANSFER_IN', data.invoice_external_id, { ...data, status: 'AGUARDANDO_CONFERENCIA_LOJA', reference_transfer_id: transaction.id });
        await (0, index_1.createBusinessRelationship)(enums_1.BANBAN_ORG_ID, 'BASED_ON_ORDER', transferInDocument.id, transaction.id, {});
    ...
    async _registerCompletion(data, metadata) {
        ...
        const destinationLocationEntity = await this._getOrCreateBusinessEntity(enums_1.BANBAN_ORG_ID, 'LOCATION', transaction.attributes.destination_location_external_id, { name: transaction.attributes.destination_location_name });
        for (const item of data.items_received) {
            const productEntity = await this._getOrCreateBusinessEntity(enums_1.BANBAN_ORG_ID, 'PRODUCT', item.product_id, { name: item.product_name || `Produto ${item.product_id}` });
            const inventoryMovement = await (0, index_1.createBusinessTransaction)(enums_1.BANBAN_ORG_ID, 'INVENTORY_MOVEMENT', null, {
            ...
            await (0, index_1.createBusinessRelationship)(enums_1.BANBAN_ORG_ID, 'AFFECTS_PRODUCT', inventoryMovement.id, productEntity.id, {});
            await (0, index_1.createBusinessRelationship)(enums_1.BANBAN_ORG_ID, 'AT_LOCATION', inventoryMovement.id, destinationLocationEntity.id, {});
            await (0, index_1.createBusinessRelationship)(enums_1.BANBAN_ORG_ID, 'CAUSED_BY_TRANSFER', inventoryMovement.id, transaction.id, {});
    ...
    async _getTransactionByExternalId(externalId, transactionType = 'TRANSFER_OUT') {
        const { data, error } = await this.supabase.from('tenant_business_transactions').select('*').eq('organization_id', enums_1.BANBAN_ORG_ID).eq('external_id', externalId).eq('transaction_type', transactionType).single();
        if (error || !data)
    ...
    async _updateInventorySnapshot(variantExternalId, locationExternalId, qtyChange, movementType, referenceId) {
        ...
        const { data: existing } = await this.supabase.from('tenant_snapshots').select('*').eq('organization_id', enums_1.BANBAN_ORG_ID).eq('snapshot_key', snapshotKey).single();
