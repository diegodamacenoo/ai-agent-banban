...
export type TransferStatus = 'PEDIDO_TRANSFERENCIA_CRIADO' | 'MAPA_SEPARACAO_CRIADO' | 'AGUARDANDO_SEPARACAO_CD' | 'EM_SEPARACAO_CD' | 'SEPARACAO_CD_COM_DIVERGENCIA' | 'SEPARACAO_CD_SEM_DIVERGENCIA' | 'SEPARADO_PRE_DOCA' | 'EMBARCADO_CD' | 'TRANSFERENCIA_CDH_FATURADA' | 'AGUARDANDO_CONFERENCIA_LOJA' | 'EM_CONFERENCIA_LOJA' | 'CONFERENCIA_LOJA_COM_DIVERGENCIA' | 'CONFERENCIA_LOJA_SEM_DIVERGENCIA' | 'EFETIVADO_LOJA';
export type TransactionType = 'ORDER_TRANSFER' | 'DOCUMENT_TRANSFER_OUT' | 'DOCUMENT_TRANSFER_IN' | 'INVENTORY_MOVEMENT' | 'TRANSFER_OUT' | 'TRANSFER_IN';
export type RelationshipType = 'ORIGINATES_FROM' | 'DESTINED_TO' | 'CONTAINS_ITEM' | 'AFFECTS_PRODUCT' | 'AT_LOCATION' | 'CAUSED_BY_TRANSFER' | 'BASED_ON_ORDER' | 'CORRESPONDS_TO';
export declare function isValidTransferStatus(status: string): status is TransferStatus;
//# sourceMappingURL=transfer-types.d.ts.map