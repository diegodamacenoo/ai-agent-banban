export class BanBanPerformanceService {
  
  async getFashionMetrics() {
    // Simular métricas específicas de moda/fashion
    return {
      totalCollections: 12,
      activeProducts: 1847,
      seasonalTrends: {
  ...
  async getInventoryTurnover(category?: string, period: string = '30d') {
    const periodDays = this.getPeriodDays(period);
    
    // Simular dados de giro de estoque por categoria
    ...
    return {
      ...
      categories: categories.map(cat => ({
        category: cat,
        turnoverRate: Math.random() * 8 + 2, // Entre 2 e 10
  ...
  async getSeasonalAnalysis(year?: number, season?: string) {
    ...
    return {
      ...
      seasons: seasons.map(s => ({
        ...
        growth: (Math.random() - 0.5) * 40, // -20% a +20%
        topCategories: [
        ...
        weatherImpact: {
          ...
          salesCorrelation: Math.random() * 0.4 + 0.6 // 0.6 a 1.0
        },
        promotions: [
      ...
      yearOverYear: {
        totalGrowth: (Math.random() - 0.3) * 25, // -7.5% a +17.5%
  ...
  async getBrandPerformance(brandId?: string, period: string = '30d', metric: string = 'revenue') {
    ...
    return {
      ...
      brands: brands.map(brand => ({
        ...
        metrics: {
          ...
          margin: Math.random() * 30 + 20 // 20% a 50%
        },
        growth: (Math.random() - 0.4) * 50, // -20% a +30%
        topProducts: [
  ...
  async getProductMargins(filters: any = {}) {
    const { productId, category, minMargin, maxMargin } = filters;
    
    // Simular produtos com diferentes margens
    const allProducts = [
    ...
    const products = filteredProducts.map(product => {
      ...
      if (minMargin && product.marginPercent < minMargin) return false;
      if (maxMargin && product.marginPercent > maxMargin) return false;
      return true;
    });
  ...
  private getOperationalStatusFromTurnover(turnover: number): string {
    if (turnover >= 8) return 'ENABLED';
    if (turnover >= 6) return 'ENABLED';
    if (turnover >= 4) return 'PENDING_APPROVAL';
    if (turnover >= 2) return 'DISABLED';
    return 'DISABLED';
  }

  private getHealthStatusFromTurnover(turnover: number): string {
    if (turnover >= 8) return 'healthy';
    if (turnover >= 6) return 'healthy';
    if (turnover >= 4) return 'warning';
    if (turnover >= 2) return 'critical';
    return 'critical';
  }
