...
// Define as ações de inventário permitidas, conforme o plano.
...
type InventoryAction =
  (typeof INVENTORY_ACTIONS)[keyof typeof INVENTORY_ACTIONS];

/**
 * Orquestra os fluxos de trabalho de inventário do BanBan.
 * Contém toda a lógica de negócio, separada da camada de rotas.
 */
export class BanBanInventoryFlowService {
  ...
  /**
   * Ponto de entrada principal para processar uma ação de inventário.
   */
  async processAction(
    ...
    console.log(
      `[Service] Iniciando ação '${action}' para a organização ${organizationId}`
    );
    await validateAndGetTenant(organizationId);
    ...
    switch (action) {
      ...
      case INVENTORY_ACTIONS.TRANSFER_INTERNAL:
        return this._transferInternal(
          organizationId,
          transactionData,
          metadata
        );
      case INVENTORY_ACTIONS.QUARANTINE_PRODUCT:
        return this._quarantineProduct(
          organizationId,
          transactionData,
          metadata
        );
  ...
  /**
   * Lida com a consulta de dados de inventário para a rota GET, com filtros avançados.
   */
  async getInventoryData(organizationId: string, queryParams: any) {
    console.log(
      `[Service] Buscando dados de inventário com filtros:`,
      queryParams
    );
    const {
      limit = 50,
    ...
    // Consulta de snapshots (estado atual)
    if (
      variant_external_id ||
      location_external_id ||
      show_reserved ||
      ...
      if (variant_external_id)
        query = query.ilike(
          "snapshot_value->>variant_external_id",
          `%${variant_external_id}%`
        );
      if (location_external_id)
        query = query.ilike(
          "snapshot_value->>location_external_id",
          `%${location_external_id}%`
        );
      if (show_reserved) query = query.gt("snapshot_value->>qty_reserved", 0);
      if (show_quarantined)
      ...
      if (error) {
        ...
        throw new Error(
          `Erro ao buscar snapshots de inventário: ${error.message}`
        );
      }
      return {
        snapshots: data || [],
    ...
    // Consulta de transações (histórico)
    console.log("[Service-GET] Modo de consulta: Transações");
    let query = this.supabase
      .from("tenant_business_transactions")
      .select("*")
      .eq("organization_id", organizationId);
    if (movement_type) query = query.eq("transaction_type", movement_type);
    ...
    if (error) {
      ...
      throw new Error(
        `Erro ao buscar transações de inventário: ${error.message}`
      );
    }
    return {
      transactions: data || [],
  ...
  /** Ajusta o estoque de um item, criando transação e evento. */
  private async _adjustStock(
    ...
    if (
      ...
      throw new Error(
        "variant_external_id, location_external_id e qty_change são obrigatórios"
      );
    }
    ...
    const productEntity = await this._getOrCreateBusinessEntity(
      ...
      "PRODUCT",
      variant_external_id,
      { name: `Produto ${variant_external_id}` }
    );
    const locationEntity = await this._getOrCreateBusinessEntity(
      organizationId,
      "LOCATION",
      location_external_id,
      { name: `Local ${location_external_id}` }
    );

    const inventoryMovement = await createBusinessTransaction(
      organizationId,
      "INVENTORY_MOVEMENT",
      external_id || crypto.randomUUID(), // external_id para o movimento
      {
    ...
    await createBusinessRelationship(
      organizationId,
      "CAUSED_BY_ADJUSTMENT",
      inventoryMovement.id,
      inventoryMovement.id, // Referencia o próprio movimento como causa
      {}
    );
  ...
  /** Registra uma contagem de inventário e seus itens. */
  private async _countInventory(
    organizationId: string,
    transactionData: any,
    metadata?: any
  ) {
    ...
    const countEntity = await upsertBusinessEntity(
      organizationId,
      "inventory_count",
      { external_id, ...transactionData, ...metadata }
    );
    await this._createBusinessEvent(
      organizationId,
      "INVENTORY_COUNT",
    ...
    for (const item of items) {
      try {
        await this._validateVariantExists(
          item.variant_external_id,
          organizationId
        );
        await upsertBusinessEntity(organizationId, "inventory_count_item", {
        ...
        console.error(
          `[_countInventory] Erro ao processar item de contagem ${item.variant_external_id}:`,
          error
        );
      }
    }
  ...
  /** Reserva estoque para um pedido ou finalidade. */
  private async _reserveStock(
    ...
    for (const item of items) {
      ...
      const snapshot = await this._getInventorySnapshot(
        organizationId,
        variant_external_id,
        location_external_id
      );
      ...
      const qty_available =
        (stock.current_stock || 0) - (stock.qty_reserved || 0);

      if (qty_available < qty_to_reserve) {
        throw new Error(
          `Estoque insuficiente para ${variant_external_id}. Disponível: ${qty_available}, Necessário: ${qty_to_reserve}`
        );
      }

      // Criar transação INVENTORY_MOVEMENT para reserva
      const inventoryMovement = await createBusinessTransaction(
        organizationId,
        "INVENTORY_MOVEMENT",
        external_id || crypto.randomUUID(), // external_id para o movimento
      ...
      await createBusinessRelationship(
        organizationId,
        "CAUSED_BY_RESERVATION",
        inventoryMovement.id,
        inventoryMovement.id, // Referencia o próprio movimento como causa
        {}
      );

      // Atualizar o snapshot (apenas qty_reserved)
  ...
  private async _transferInternal(
    organizationId: string,
    transactionData: any,
    metadata?: any
  ) {
    ...
    if (
      ...
      throw new Error(
        "items, origin_location e destination_location são obrigatórios."
      );
    ...
    const originLocationEntity = await this._getOrCreateBusinessEntity(
      ...
      "LOCATION",
      origin_location_external_id,
      { name: `Local ${origin_location_external_id}` }
    );
    const destinationLocationEntity = await this._getOrCreateBusinessEntity(
      organizationId,
      "LOCATION",
      destination_location_external_id,
      { name: `Local ${destination_location_external_id}` }
    );

    const transferTransaction = await createBusinessTransaction(
      organizationId,
      "INTERNAL_TRANSFER",
      external_id,
      transactionData
    ...
    for (const item of items) {
      ...
      console.log(
        `[_transferInternal] Transferindo ${qty_transfer} de ${variant_external_id} de ${origin_location_external_id} para ${destination_location_external_id}`
      );

      // Movimento de saída do local de origem
      const outboundMovement = await createBusinessTransaction(
        organizationId,
        "INVENTORY_MOVEMENT",
      ...
      // Movimento de entrada no local de destino
      const inboundMovement = await createBusinessTransaction(
        organizationId,
        "INVENTORY_MOVEMENT",
        null, // external_id pode ser nulo para movimentos internos
        {
  ...
  /** Coloca um produto em quarentena, tornando-o indisponível. */
  private async _quarantineProduct(
    organizationId: string,
    transactionData: any,
    metadata?: any
  ) {
    ...
    if (!variant_external_id || !location_external_id || !qty_quarantine)
      throw new Error(
        "variant_external_id, location_external_id e qty_quarantine são obrigatórios."
      );
    ...
    // Criar transação INVENTORY_MOVEMENT para quarentena
    const inventoryMovement = await createBusinessTransaction(
      organizationId,
      "INVENTORY_MOVEMENT",
      external_id || crypto.randomUUID(), // external_id para o movimento
  ...
  /** Busca o snapshot de inventário para um item/local. */
  private async _getInventorySnapshot(
    organizationId: string,
  ...
  /** Cria um evento de negócio para fins de auditoria. */
  ...
  /** Valida a existência de uma localização. */
  private async _validateLocationExists(
    locationExternalId: string,
    organizationId: string
  ) {
  ...
  /** Valida a existência de uma variação de produto. */
  private async _validateVariantExists(
    variantExternalId: string,
    organizationId: string
  ) {
  ...
  /** Atualiza o snapshot de inventário de forma centralizada. */
  private async _updateInventorySnapshot(
    ...
    const stock =
      existingStock ||
      (
        await this._getInventorySnapshot(
          organizationId,
          variantExternalId,
          locationExternalId
        )
  ...
  /** Verifica e cria alertas de estoque baixo. */
  private async _checkStockAlerts(
    organizationId: string,
    variantExternalId: string,
    locationExternalId: string,
    currentStock: number
    ...
    if (currentStock < 10 && currentStock >= 0) {
      console.log(
        `[_checkStockAlerts] Alerta de estoque baixo para ${variantExternalId} no local ${locationExternalId}. Estoque: ${currentStock}`
      );
      const alertSnapshot = {
