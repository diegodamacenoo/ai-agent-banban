...
import { BanBanInventoryFlowService } from './services/banban-inventory-flow-service';
import { banbanInventoryFlowSchemas } from './schemas/banban-inventory-flow-schemas';

export class BanBanInventoryFlowModule implements ModuleInstance {
  name = 'banban-inventory-flow';
  version = '2.0.0';
  description = 'Módulo de fluxo de inventário migrado das edge functions para BanBan Fashion';
  baseModule = 'inventory-base';
  
  private service: BanBanInventoryFlowService;

  constructor() {
    this.service = new BanBanInventoryFlowService();
  }

  async register(fastify: FastifyInstance, prefix?: string): Promise<void> {
    // Registrar schemas específicos do Inventory Flow
    fastify.addSchema(banbanInventoryFlowSchemas.inventoryEvent);
    fastify.addSchema(banbanInventoryFlowSchemas.inventoryFlowWebhook);
    fastify.addSchema(banbanInventoryFlowSchemas.inventoryFlowResponse);
    fastify.addSchema(banbanInventoryFlowSchemas.inventoryAnalytics);
    ...
    // 1. Main Inventory Flow Webhook Endpoint (POST)
    ...
    // 2. Inventory Events Query Endpoint (GET)
    ...
    // 3. Inventory Flow Analytics Endpoint
    ...
    // 4. Health check específico do módulo Inventory Flow
