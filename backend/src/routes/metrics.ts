...
export async function metricsRoutes(fastify: FastifyInstance) {
  const metricsCollector = (fastify as any).metricsCollector as MetricsCollector;

  // Endpoint raiz de métricas (overview)
  ...
  }>('/metrics/history', async (request, reply) => {
    try {
      const minutes = parseInt(request.query.minutes || '60');
      
      if (isNaN(minutes) || minutes < 1 || minutes > 1440) { // Max 24 horas
  ...
  // Endpoint para métricas agregadas (para dashboard)
  fastify.get<{
    Querystring: { minutes?: string }
  }>('/metrics/aggregated', async (request, reply) => {
    try {
  ...
  // Endpoint para status de saúde do sistema
  ...
  // Endpoint para métricas por tenant (futuro)
  fastify.get<{
    Params: { tenantId: string }
  }>('/metrics/tenant/:tenantId', async (request, reply) => {
    try {
  ...
  // Endpoint para resetar métricas (desenvolvimento apenas)
  if (process.env.NODE_ENV === 'development') {
    fastify.post('/metrics/reset', async (request, reply) => {
      try {
        // Reinicializar o collector
        const newCollector = new MetricsCollector(fastify);
        (fastify as any).metricsCollector = newCollector;
