-- Fix function return types to match existing schema
-- Migration: 20250121140000_fix_function_return_types.sql
...
DROP FUNCTION IF EXISTS user_can_access_module(UUID, TEXT, UUID);

-- ================================================
-- FUNÇÃO CONSOLIDADA DE VISIBILIDADE (TIPOS CORRETOS)
-- ================================================
...
RETURNS TABLE(
  module_slug VARCHAR(50),  -- Match base_modules.slug type
  module_name VARCHAR(100), -- Match base_modules.name type
  module_category VARCHAR(50), -- Match base_modules.category type
  can_view BOOLEAN,
  can_access BOOLEAN,
  assignment_id UUID,
  custom_config JSONB,
  ...
  status TEXT,
  implementation_key VARCHAR(50), -- Match module_implementations.implementation_key type
  component_path TEXT
) 
LANGUAGE plpgsql
SECURITY DEFINER
...
-- ================================================
-- FUNÇÃO SIMPLIFICADA PARA API (TIPOS CORRETOS)
-- ================================================
...
-- ================================================
-- FUNÇÃO PARA VERIFICAR PERMISSÕES ESPECÍFICAS (TIPOS CORRETOS)
-- ================================================

-- Function to check if user has permission for specific module
CREATE OR REPLACE FUNCTION user_can_access_module(
  p_tenant_id UUID,
  p_module_slug VARCHAR(50), -- Match the actual column type
  p_user_id UUID DEFAULT NULL
)
RETURNS BOOLEAN
LANGUAGE plpgsql
...
'Função consolidada que retorna todos os módulos visíveis para um tenant com tipos corrigidos para corresponder ao schema';

COMMENT ON FUNCTION get_visible_modules_for_tenant(UUID) IS 
'Função simplificada que retorna apenas módulos visíveis e acessíveis para uso em APIs com tipos corretos';

COMMENT ON FUNCTION user_can_access_module(UUID, VARCHAR(50), UUID) IS 
'Função para verificar se um usuário específico pode acessar um módulo específico com tipos corretos';
