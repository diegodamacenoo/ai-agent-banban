-- Fix component_path type in function
-- Migration: 20250121142000_fix_component_path_type.sql
...
-- ================================================
-- FUNÇÃO CONSOLIDADA DE VISIBILIDADE (TIPOS CORRETOS v3)
-- ================================================
...
RETURNS TABLE(
  ...
  component_path VARCHAR(255) -- Corrected type to match actual column
) 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  SELECT 
    ...
    mi.component_path::VARCHAR(255) -- Cast to match return type
...
-- ================================================
-- FUNÇÃO SIMPLIFICADA PARA API (TIPOS CORRETOS v3)
-- ================================================
...
RETURNS TABLE(
  module_slug VARCHAR(50),
  module_name VARCHAR(100),
  component_path VARCHAR(255), -- Match the corrected type
  custom_config JSONB
)
LANGUAGE plpgsql
SECURITY DEFINER
...
-- ================================================
-- FUNÇÃO PARA VERIFICAR PERMISSÕES ESPECÍFICAS (TIPOS CORRETOS v3)
-- ================================================
...
'Função consolidada que retorna todos os módulos visíveis para um tenant com tipos corrigidos v3';

COMMENT ON FUNCTION get_visible_modules_for_tenant(UUID) IS 
'Função simplificada que retorna apenas módulos visíveis e acessíveis para uso em APIs v3';

COMMENT ON FUNCTION user_can_access_module(UUID, VARCHAR(50), UUID) IS 
'Função para verificar se um usuário específico pode acessar um módulo específico v3';
