...
const sidebarVariants = cva(
  "z-40 bg-background border-r border-[hsl(var(--sidebar-border))] flex flex-col",
  {
    variants: {
      variant: {
...
const sidebarItemVariants = cva(
  "relative flex items-center rounded-lg text-sm font-medium cursor-pointer group",
  {
    variants: {
      variant: {
        default: "text-muted-foreground hover:text-foreground hover:bg-hu-accent",
        active: "text-hu-primary bg-hu-accent hover:bg-hu-accent",
        ghost: "text-muted-foreground hover:text-foreground hover:bg-hu-accent/50",
      },
      collapsed: {
...
const SidebarItem: React.FC<SidebarItemProps> = ({
  ...
  const ItemContent = (
    <>
      ...
      {collapsed && (
        <div
          className="absolute left-full ml-2 px-2 py-1 bg-popover text-popover-foreground text-xs rounded-lg border border-border opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 whitespace-nowrap z-50"
          role="tooltip"
          id={`${itemId}-tooltip`}
        >
          {label}
