...
/**
 * Schema de validação para restauração de usuário.
 * @description Valida os dados necessários para restaurar um usuário excluído (soft delete).
 */
...
/**
 * API Route para restauração de usuários excluídos (soft delete).
 * 
 * @description Restaura um usuário específico da organização que foi excluído via soft delete.
 * Apenas administradores da organização podem restaurar usuários.
 * 
 ...
 * - Verifica se usuário está realmente excluído
...
export async function POST(request: Request) {
  try {
    ...
    const validation = restoreUserSchema.safeParse(body);
    ...
    if (currentUserProfile.role !== 'organization_admin') {
      return NextResponse.json({ error: 'Apenas administradores podem restaurar usuários' }, { status: 403 });
    }
    ...
    if (targetUserError || !targetUser) {
      console.error('Erro ao buscar usuário a ser restaurado:', targetUserError);
    ...
    if (targetUser.organization_id !== currentUserProfile.organization_id) {
      return NextResponse.json({ error: 'Não é possível restaurar usuário de outra organização' }, { status: 403 });
    }

    // Verificação se o usuário está realmente excluído
    if (!targetUser.deleted_at) {
      return NextResponse.json({ error: 'Usuário não está excluído' }, { status: 400 });
    }

    // Execução da restauração (remoção do deleted_at)
    ...
    if (restoreError) {
      console.error('Erro ao restaurar usuário:', restoreError);
      return NextResponse.json({ error: `Erro ao restaurar usuário: ${restoreError.message}` }, { status: 500 });
    }
    ...
    try {
      ...
      await createAuditLog({
        ...
        details: {
          restored_user_email: userEmail,
          restored_user_name: `${targetUser.first_name} ${targetUser.last_name}`.trim(),
          restored_user_role: targetUser.role
        }
      });
    } catch (auditError) {
