...
export default function ModulesPage() {
  ...
  useEffect(() => {
    console.log('📋 PAGE: Lista de baseModules atualizada:', baseModules.length, 'módulos');
    baseModules.forEach(module => console.log('  -', module.name, module.id));
  }, [baseModules]);
  ...
  const loadData = useCallback(async () => {
    ...
    if (!mountedRef.current && mountedRef.current !== false) {
      console.log('🚫 CLIENT: Component not mounted, skipping loadData');
      return;
    }
    
    const callId = `LOAD_${Date.now()}`;
    console.log(`🚀 CLIENT: Starting loadData ${callId}`);
    ...
    try {
      ...
      console.log(`✅ CLIENT: Completed loadData ${callId}`);
    }
  ...
  useEffect(() => {
    ...
    if (loadCalledRef.current) {
      console.log('🚫 CLIENT: loadData já foi chamado, pulando...');
      return;
    }
  ...
  const updateModuleConfig = async (tenantId: string, baseModuleId: string, config: Record<string, any>) => {
    try {
      ...
      if (currentAssignment && handleOptimisticAssignmentUpdate) {
        ...
        console.log('🚀 Update otimístico de config aplicado:', operationId);
        ...
        if (result.success) {
          ...
          console.log('✅ Update de config confirmado pelo servidor:', operationId);
        } else {
  ...
  const handleBaseModuleChange = useCallback(() => {
    ...
    console.log('🚀 Mudança de base module detectada - usando estado otimístico');
  }, []);

  const handleOptimisticBaseModuleCreate = useCallback((newBaseModule) => {
    console.log('📝 PAGE: handleOptimisticBaseModuleCreate chamado para:', newBaseModule.name);
    const operationId = optimisticCreateBaseModule(newBaseModule);
    console.log('📝 PAGE: operationId retornado:', operationId);
    return operationId;
  }, [optimisticCreateBaseModule]);

  const handleOptimisticBaseModuleUpdate = useCallback((updatedBaseModule) => {
    console.log('✏️ PAGE: handleOptimisticBaseModuleUpdate chamado para:', updatedBaseModule.name);
    const operationId = optimisticUpdateBaseModule(updatedBaseModule);
    console.log('✏️ PAGE: operationId retornado:', operationId);
    return operationId;
  ...
  const handleBaseModuleServerOperationSuccess = useCallback((operationId, serverData = null) => {
    console.log('✅ PAGE: handleBaseModuleServerOperationSuccess chamado:', operationId, serverData?.name);
  ...
  const handleBaseModuleServerOperationError = useCallback((operationId, errorMessage) => {
    console.log('❌ PAGE: handleBaseModuleServerOperationError chamado:', operationId, errorMessage);
  ...
  const handleImplementationChange = useCallback(() => {
    ...
    console.log('🚀 Mudança de implementação detectada - usando estado otimístico');
  }, []);
  ...
  const handleAssignmentChange = useCallback(() => {
    ...
    console.log('🚀 Mudança de assignment detectada - usando estado otimístico');
  }, []);
